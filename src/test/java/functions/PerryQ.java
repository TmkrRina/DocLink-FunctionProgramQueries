...
List<User>users;
     ...
     List<Category> categories = new ArrayList<Category>(){ { add(Category.HEART);add(Category.CHEST);add(Category.DENTAL); } };
    ...
    public void get_Doctor_Form_Post(){//good
        hashMap2.put(1,5);
        hashMap2.put(2,1);
       System.out.println(FunctionForQ.get_doctor_from_post.apply(posts));
        assertEquals(hashMap2,FunctionForQ.get_doctor_from_post.apply(posts));

    }
    @Test
    public void getGet_doctor_from_commnts(){//good
      //  System.out.println("test"+FunctionForQ.getGet_doctor_from_commnts.apply(comments));
    ...
    public void Kth_Doctors_That_Active(){//good
        //System.out.println(FunctionForQ.Kth_Doctors_That_Active.apply(FunctionForQ.get_doctor_from_post.apply(posts),FunctionForQ.getGet_doctor_from_commnts.apply(comments),1,doctors));
...
hashMap2.put(Category.SKIN,null);
    hashMap2.put(Category.EAR_NOSE_THROAT,null);
    hashMap2.put(Category.EYES,null);
    hashMap2.put(Category.DENTAL,testPosts);
        assertEquals(testPosts,FunctionForQ.categories.apply(testPosts));

}
@Test
    public void common_health_issue(){
List<AbstractMap.SimpleEntry<Object, Integer>> str = new java.util.ArrayList<>();
    AbstractMap.SimpleEntry<Object, Integer> pair1= new AbstractMap.SimpleEntry<Object, Integer>(Category.DENTAL,15);
    str.add(pair1);
